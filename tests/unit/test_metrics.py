...
from coreax.metrics import CMMD, EMCMD, JMMD, KSD, MMD
...
class TestCMMD:
    ...
    def test_cmmd_compare_same_data(
        self, problem: _SupervisedMetricProblem, kernel: ScalarValuedKernel
    ):
...
class TestEMCMD:
    """
    Tests for the expected conditional maximum mean discrepancy (EMCMD) class.
    """

    @pytest.fixture
    def problem(self) -> _SupervisedMetricProblem:
        """Generate an example problem for testing EMCMD."""
        feature_dimension = 1
        response_dimension = 1
        num_points = 2500, 1000
        keys = tuple(jr.split(jr.key(0), 2))
    ...
    def test_emcmd_compare_same_data(
        self, problem: _SupervisedMetricProblem, kernel: ScalarValuedKernel
    ):
        """Check EMCMD of a dataset with itself is approximately zero."""
        x = problem.reference_data
        metric = EMCMD(
            feature_kernel=kernel, response_kernel=kernel, regularisation_parameter=1e-2
        )
        assert metric.compute(x, x) == pytest.approx(0.0)

    # pylint: disable=too-many-locals
    def test_emcmd_random_data(self, problem: _SupervisedMetricProblem):
        r"""
        Test EMCMD computed from randomly generated test data agrees with method result.
        """
        kernel = SquaredExponentialKernel()
        reference_data, comparison_data = problem
        regularisation_parameter = 1e-2
        ...
        least_square_solution_1 = ExactSolver().solve(
            array=feature_gramian_1,
            regularisation_parameter=regularisation_parameter,
            identity=jnp.eye(feature_gramian_1.shape[0]),
            target=feature_gramian_1,
        )

        least_square_solution_2 = ExactSolver().solve(
            array=feature_gramian_2,
            regularisation_parameter=regularisation_parameter,
            identity=jnp.eye(feature_gramian_2.shape[0]),
            target=kernel.compute(x2, x1),
        )

        # Compute each term in the EMCMD
        term_1 = jnp.sum(
            kernel.compute(y1, y1) @ least_square_solution_1 * least_square_solution_1
        )
        term_2 = jnp.sum(
            kernel.compute(y2, y2) @ least_square_solution_2 * least_square_solution_2
        )
        term_3 = jnp.sum(
            kernel.compute(y1, y2) @ least_square_solution_2 * least_square_solution_1
        )

        # Compute EMCMD
        expected_cmmd = jnp.sqrt(term_1 + term_2 - 2 * term_3)
    ...
    def test_approximate_emcmd_random_data(
        self,
        problem: _SupervisedMetricProblem,
        least_squares_solver: RegularisedLeastSquaresSolver,
    ):
        r"""
        Test approximate EMCMD is close to exact EMCMD.
        """
        kernel = SquaredExponentialKernel()
        x, y = problem

        # Compute exact and approximate EMCMD
        regularisation_parameter = 1e-2
        metric = EMCMD(
            feature_kernel=kernel,
            response_kernel=kernel,
