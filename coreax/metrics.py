...
class JMMD(Metric[SupervisedData]):
    ...
    kernel: coreax.kernels.TensorProductKernel
    precision_threshold: float = 1e-12

    def __init__(
        self,
        feature_kernel: coreax.kernels.ScalarValuedKernel,
        response_kernel: coreax.kernels.ScalarValuedKernel,
        precision_threshold: float = 1e-12,
    ):
        """Initialise JMMD class and build tensor-product kernel."""
        self.kernel = coreax.kernels.TensorProductKernel(
...
class CMMD(Metric[SupervisedData]):
    ...
    def compute(
        ...
        **kwargs,
        ...
        reference_size, comparison_size = len(reference_data), len(comparison_data)
        ...
        if self.least_squares_solver is None:
            least_squares_solver = ExactSolver()
        else:
        ...
        inverse_feature_gramian_1 = least_squares_solver.solve(
            ...
            regularisation_parameter=self.regularisation_parameter * reference_size,
            identity=jnp.eye(reference_size),
            target=jnp.eye(reference_size),
        )

        inverse_feature_gramian_2 = least_squares_solver.solve(
            array=feature_gramian_2,
            regularisation_parameter=self.regularisation_parameter * comparison_size,
            identity=jnp.eye(comparison_size),
            target=jnp.eye(comparison_size),
        )
...
class AMCMD(Metric[SupervisedData]):
    ...
    :math:`\mathcal{D}^{(2)} = \{(\tilde{x}_i, \tilde{y}_i)\}_{i=1}^m` of ``m`` pairs
    with :math:`\tilde{x}\in\mathbb{R}^d` and :math:`\tilde{y}\in\mathbb{R}^p`,
    the average maximum conditional mean discrepancy computed with respect to another
    dataset :math:`\mathcal{D}^{(0)} = \{(x_i)\}_{i=1}^q` is given by:

    .. math::
        \text{AMCMD}^2(\mathcal{D}^{(0)}, \mathcal{D}^{(1)}, \mathcal{D}^{(2)}) =
        \frac{1}{q}\sum_{i=1}^q
        ||\hat{\mu}^{(1)}_{Y|X=x_i} - \hat{\mu}^{(2)_{Y|X=x_i}}||^2_{\mathcal{H}_l}

    where :math:`\hat{\mu}^{(1)}_{Y|X}, \hat{\mu}^{(2)}_{Y|X}` are the conditional mean
    embeddings (:cite:`muandet2016rkhs`) estimated with :math:`\mathcal{D}^{(1)}` and
    :math:`\mathcal{D}^{(2)}` respectively, and :math:`\mathcal{H}_k, \mathcal{H}_l` is
    the RKHS induced by the kernel :math:`l: \mathbb{R}^p \times \mathbb{R}^p
    \rightarrow \mathbb{R}` respectively.


    .. note::
        It can be shown that if :math:`\mu^{(1)}_{Y|X=x} = \mu^{(2)}_{Y|X=x}`, then
        :math:`\mathbb{P}^{(1)}_{Y|X} = \mathbb{P}^{(2)}_{Y|X}` in the sense that almost
        everywhere :math:`x` with respect to :math:`\mathbb{P}^{(0)}_X`, we have
        :math:`\mathbb{P}^{(1)}_{Y|X=x} = \mathbb{P}^{(2)}_{Y|X=}`.
        Therefore, the AMCMD gives us a way to measure if two supervised datasets have
        the same (in the sense above) conditional distributions.
    ...
    :param regularisation_parameter: Regularisation parameter for stable inversion
            of arrays, negative values will be converted to positive
    :param: The unsupervised dataset corresponding to the first argument of the AMCMD.
        Defaults to :data:`None`, using the features of the ``reference_data``.
    :param precision_threshold: Positive threshold we compare against for precision
    :param least_squares_solver: Instance of
        :class:`coreax.least_squares.RegularisedLeastSquaresSolver`, defaults to
        :class:`coreax.least_squares.ExactSolver` which solves a linear
    ...
    response_kernel: coreax.kernels.ScalarValuedKernel
    regularisation_parameter: float
    weighting_data: Optional[Data] = None
    precision_threshold: float = 1e-2
    least_squares_solver: Optional[RegularisedLeastSquaresSolver] = None
    ...
    def compute(
        ...
        **kwargs,
        ...
        r"""
        Compute the average maximum mean discrepancy between the datasets.
        ...
        `comparison_data` of ``n`` pairs with
        :math:`\tilde{x}\in\mathbb{R}^d` and :math:`\tilde{y}\in \mathbb{R}^p`, the
        average maximum mean discrepancy is given by:

        .. math::
        \text{AMCMD}^2(\mathcal{D}^{(1)}, \mathcal{D}^{(2)}) =
        \mathbb{E}_{x\sim\mathbb{P}}\left[]
        ||\hat{\mu}^{(1)}_{Y|X=x} - \hat{\mu}^{(2)_{Y|X=x}}||^2_{\mathcal{H}_l}\right]

        where :math:`\hat{\mu}^{(1)}_{Y|X}, \hat{\mu}^{(2)}_{Y|X}` are the conditional
        mean embeddings (:cite:`muandet2016rkhs`) estimated with
        :math:`\mathcal{D}^{(1)}` and :math:`\mathcal{D}^{(2)}` respectively, and
        :math:`\mathcal{H}_k, \mathcal{H}_l` is the RKHS induced by the kernel
        :math:`l: \mathbb{R}^p \times \mathbb{R}^p \rightarrow \mathbb{R}` respectively.

        .. note::
            The expectation is estimated with the features from the ``reference_data``.
        ...
        # Extract the array of weighting data
        if self.weighting_data is None:
            x0 = reference_data.data
            weighting_size = reference_size
        else:
            x0 = self.weighting_data.data
            weighting_size = len(self.weighting_data)

        # Compute feature gramians
        feature_gramian_1 = self.feature_kernel.compute(x1, x1)
        feature_gramian_2 = self.feature_kernel.compute(x2, x2)

        # Compute cross feature gramians
        cross_feature_gramian_10 = self.feature_kernel.compute(x1, x0)
        cross_feature_gramian_20 = self.feature_kernel.compute(x2, x0)
        ...
        least_square_solution_1 = least_squares_solver.solve(
            array=feature_gramian_1,
            regularisation_parameter=self.regularisation_parameter,
            identity=jnp.eye(reference_size),
            target=cross_feature_gramian_10,
        )

        least_square_solution_2 = least_squares_solver.solve(
            array=feature_gramian_2,
            regularisation_parameter=self.regularisation_parameter,
            identity=jnp.eye(comparison_size),
            target=cross_feature_gramian_20,
        )
        ...
        squared_amcmd_threshold_applied = (
            coreax.util.apply_negative_precision_threshold(
                1 / weighting_size * (term_1 + term_2 - 2 * term_3),
                self.precision_threshold,
            )
        )
